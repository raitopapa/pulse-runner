...
自動テストは入力猶予、ジャンプ高度、ダッシュ、壁蹴り、セーブ検証、落下からの復帰、4ステージの穴越え、武器挙動、4ボスの進行、PWAキャッシュを対象にしています。移行前の公開URLのChromeで起動、ステージ選択、射撃・ジャンプのボタン、一時停止／再開、設定変更、オフライン準備完了表示を確認しました。390×844、844×390、1024×768のiframeで縦横の画面配置を確認しています。v2の自動テストは40件です。追加検証は武器別装置起動・往復刃の盾背面攻撃・火炎車輪の接地貫通・凍結足場・弾数独立・永続強化・全ボス固有技・警告時間・楽曲スケジュールを対象とします。iPad／Android実機での長時間プレイ・複数同時タッチ・音の聴感・ゲームパッドは実機未確認です。
